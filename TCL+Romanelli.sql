...
-- CREACION DE TABLA BITACORA PARA LAS PELICULAS AGREGADAS Y ELIMINADAS

CREATE TABLE new_peliculas (
...
#DROP TRIGGER `tr_DELETE_new_calificacion`

-- TRIGGER AFTER Y BEFORE QUE GUARDAN LAS INTERACCIONES DE LA TABLA.

CREATE TRIGGER `tr_add_new_pelicula`
AFTER INSERT ON `peliculas`
FOR EACH ROW 
INSERT INTO `new_peliculas` (id_pelicula, título, fecha, hora, usuario, accion) VALUES (NEW.id_pelicula, NEW.título, CURDATE(), CURTIME(), USER(), "ADD" );

CREATE TRIGGER `tr_rest_new_pelicula`
BEFORE DELETE ON `peliculas`
FOR EACH ROW 
INSERT INTO `new_peliculas` (id_pelicula, título, fecha, hora, usuario, accion) VALUES (OLD.id_pelicula, OLD.título, CURDATE(), CURTIME(), USER(), "DELETE" );

-- TRIGGER PARA CONTROL DE CAMBIOS DE LA TABLA CALIFICACIONES
...
-- TRIGGER PARA CONTROL DE CAMBIOS DE LA TABLA CALIFICACIONES, tanto INSERT; DELETE y UPDATE

CREATE TRIGGER `tr_add_new_calificacion`
AFTER INSERT ON `calificaciones`
FOR EACH ROW 
INSERT INTO `new_calificacion` (id_calificacion, id_pelicula, comentario, calificacion, fecha, hora, usuario, accion) 
	VALUES (NEW.id_calificacion, NEW.id_pelicula, NEW.comentario, NEW.calificacion, CURDATE(), CURTIME(), USER(), "ADD" );
     
CREATE TRIGGER `tr_delete_new_calificacion`
BEFORE DELETE ON `calificaciones`
FOR EACH ROW 
INSERT INTO `new_calificacion` (id_calificacion, id_pelicula, comentario, calificacion, fecha, hora, usuario, accion) 
	VALUES (OLD.id_calificacion, OLD.id_pelicula, OLD.comentario, OLD.calificacion, CURDATE(), CURTIME(), USER(), "DELETE" );
    
CREATE TRIGGER `tr_update_new_calificacion`
BEFORE UPDATE ON `calificaciones`
FOR EACH ROW 
INSERT INTO `new_calificacion` (id_calificacion, id_pelicula, comentario, calificacion, fecha, hora, usuario, accion) 
	VALUES (OLD.id_calificacion, OLD.id_pelicula, OLD.comentario, OLD.calificacion, CURDATE(), CURTIME(), USER(), "UPDATE" );
...
-- Permisos de Insertar y Modificar:
GRANT SELECT, INSERT, UPDATE ON * TO 'admin@localhost';

-- SUBLENGUAJE TCL

-- ELIMINACION DE REGISTROS DE GENEROS, LUEGO HAGO ROLLBACK

USE cinemapy;
START TRANSACTION;

DELETE FROM generos
	WHERE id_genero IN (1, 4, 6);
...
-- INSERT DE DATOS CON SAVE POINTS

USE cinemapy;
START TRANSACTION;

INSERT INTO personas (id_persona, nombre, apellido, nacionalidad, fecha_nac)
	VALUES (11 ,'Tom ', 'Hardy', 'Inglaterra', '1977-9-15'),
	(12 ,'Joseph', 'Gordon Levitt', 'USA', '1981-2-17'),
	(13 ,'Jennifer', 'Lawrence', 'USA', '1990-8-15'),
	(14 ,'Jonah', 'Hill', 'USA', '1983-12-20');
SAVEPOINT primer_lote;

INSERT INTO personas (id_persona, nombre, apellido, nacionalidad, fecha_nac)
	VALUES (15 ,'Timothee', 'Chalamet', 'USA', '1995-12-27'),
	(16 ,'Cate', 'Blanchett', 'Australia', '1969-5-14'),
	(17 ,'Kerry', 'Washington', 'USA', '1977-1-31'),
	(18 ,'Christoph ', 'Waltz', 'Austria', '1956-10-4'),
	(19 ,'Samuel', 'Jackson', 'USA', '1948-12-21');
